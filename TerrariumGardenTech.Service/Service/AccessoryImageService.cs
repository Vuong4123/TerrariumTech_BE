...
public class AccessoryImageService(UnitOfWork _unitOfWork) : IAccessoryImageService
{
    ...
    public async Task<IBusinessResult> UpdateAccessory(AccessoryImageUpdateRequest accessoryImageUpdateRequest)
    {
        try
        {
            var result = -1;
            var accessoryImage =
                await _unitOfWork.AccessoryImage.GetByIdAsync(accessoryImageUpdateRequest.AccessoryImageId);
            if (accessoryImage != null)
            {
                _unitOfWork.AccessoryImage.Context().Entry(accessoryImage).CurrentValues
                    .SetValues(accessoryImageUpdateRequest);
