...
public class VnPayService : IVnPayService
{
    ...
    public async Task<IBusinessResult> PaymentExecute(IQueryCollection collections)
    {
        ...
        if (!response.Success)
        {
            return new BusinessResult(Const.FAIL_READ_CODE, "Payment failed: Invalid signature or response code.");
        }

        if (!int.TryParse(response.OrderId, out var orderId))
        {
            return new BusinessResult(Const.FAIL_READ_CODE, "Invalid OrderId in payment response.");
        }
