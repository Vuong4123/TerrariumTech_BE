        ...
        public async Task<(int, string, string, string)> LoginAsync(string username, string password)
        {
            try
            {
                ...
                var claims = new[] {
                    new Claim(JwtRegisteredClaimNames.Sub, user.UserId.ToString()),
                    new Claim(JwtRegisteredClaimNames.UniqueName, user.Username),
                    new Claim(ClaimTypes.Role, user.Role?.RoleName ?? RoleStatus.User.ToString()), // Sử dụng enum RoleStatus.User
                    new Claim("email", user.Email ?? ""),
                    new Claim("fullName", user.FullName ?? ""),
                    new Claim("phoneNumber", user.PhoneNumber ?? ""),
                    new Claim("gender", user.Gender ?? ""),
        ...
        public async Task<(int, string, string)> RefreshTokenAsync(string refreshToken)
        {
            try
            {
                ...
                var claims = new[] {
                    new Claim(JwtRegisteredClaimNames.Sub, user.UserId.ToString()),
                    new Claim(JwtRegisteredClaimNames.UniqueName, user.Username),
                    new Claim(ClaimTypes.Role, user.Role?.RoleName ?? RoleStatus.User.ToString()),  // Sử dụng RoleStatus.User thay vì chuỗi "User"
                    new Claim("email", user.Email ?? ""),
                    new Claim("fullName", user.FullName ?? ""),
                    new Claim("phoneNumber", user.PhoneNumber ?? ""),
                    new Claim("gender", user.Gender ?? ""),
