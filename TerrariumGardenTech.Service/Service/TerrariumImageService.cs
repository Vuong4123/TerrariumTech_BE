...
public class TerrariumImageService : ITerrariumImageService
{
    ...
    public async Task<IBusinessResult> UpdateTerrariumImageAsync(
        TerrariumImageUpdateRequest terrariumImageUpdateRequest)
    {
        try
        {
            var result = -1;
            var terraImage =
                await _unitOfWork.TerrariumImage.GetByIdAsync(terrariumImageUpdateRequest.TerrariumImageId);
            if (terraImage != null)
            {
                _unitOfWork.TerrariumImage.Context().Entry(terraImage).CurrentValues
                    .SetValues(terrariumImageUpdateRequest);
    ...
    public async Task<IBusinessResult> DeleteTerrariumImageAsync(int environmentId)
    {
        ...
        if (result != null)
        {
            ...
            if (deleteResult) return new BusinessResult(Const.SUCCESS_DELETE_CODE, Const.SUCCESS_DELETE_MSG);
    ...
    public async Task<IBusinessResult> CreateTerrariumImageAsync(
        TerrariumImageCreateRequest terrariumImageCreateRequest)
    {
        try
        {
            ...
            if (result > 0) return new BusinessResult(Const.SUCCESS_CREATE_CODE, Const.SUCCESS_CREATE_MSG, terraImage);
